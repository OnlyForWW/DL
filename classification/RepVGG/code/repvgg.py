...
# Chinese code annotation completed by Wang Junyan
...
class RepVGG(nn.Module):
    ...
    def forward(self, x):
        ...
        for stage in (self.stage1, self.stage2, self.stage3, self.stage4):
            for block in stage:
                # 该技术的核心是一种使用时间换空间的策略。详情可见：https://zhuanlan.zhihu.com/p/455541708
...
if __name__ == "__main__":
    ...
    model = RepVGG(num_blocks=[2, 4, 14, 1], num_classes=6,
                  width_multiplier=[1, 1, 1, 2.5], override_groups_map=None, deploy=deploy, use_checkpoint=use_checkpoint)
    model.eval()
    print(model)
    input = torch.randn(32, 3, 150, 150)
    y = model(input)
    print(y.size())